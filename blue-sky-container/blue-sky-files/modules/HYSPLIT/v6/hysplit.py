...
class InputARL(Process):
    ...
    def run(self, context):
        ...
        if self.config("USE_INDEXED_ARL_DATA", bool):
            ...
            metfiles = catalog_indexed_data.get_input_files(date_naive, hours_to_run)
    ...
    def ARLsize(self,f):
        ...
        domainInfo["beta"] = arlfile.mainHeader.cone_angle
        domainInfo["gamma"] = arlfile.mainHeader.tangent_long
        domainInfo["nxCRS"] = arlfile.mainHeader.numb_x_pnts    # east-west grid dimensions
        ...
        elif domainInfo["domainID"].strip() in ['AWRF']:  # Intended for UW PacNW 4-km WRF domain
            domainInfo["lon_min"] = -128.0
            domainInfo["lon_max"] = -106.0
            domainInfo["lat_min"] = 39.0
            domainInfo["lat_max"] = 51.0
        else: # Assume a CONUS extent
            domainInfo["lon_min"] = -130.0
            domainInfo["lon_max"] = -65.0
            domainInfo["lat_min"] = 20.0
            domainInfo["lat_max"] = 55.0
...
class HYSPLITTrajectory(Trajectory):
    ...
    def run(self, context):
        self.log.info("Running the HYSPLIT Trajectory model")
...
class HYSPLITDispersion(Dispersion):
    ...
    def writeControlFile(self, filteredFires, metInfo, modelStart, hoursToRun, controlFile, concFile, num_quantiles):
        ...
        # Calculate output concentration grid from met_domain_info object
        #centerLat = metInfo.met_domain_info.latC
    ...
    def writeSetupFile(self, filteredFires, modelStart, emissionsFile, setupFile, num_quantiles, ninit_val):
        ...
        with open(setupFile, "w") as f:
            ...
            # make the 'smoke initialization' files?
