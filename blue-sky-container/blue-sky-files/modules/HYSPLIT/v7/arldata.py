...
header_parser = HeaderParser(
    '(6I2,2A1,A4,I4,2E14.7)',
    [ "year", "month", "day", "hour", "forecast", "level", "gridx", "gridy",
      "variable", "exponent", "precision", "value" ]
    )
...
index_level_parser = HeaderParser( '(F6,I2)',
                                   ["height", "num_vars"] )
index_var_parser = HeaderParser( '(A4,I3,1X)',
...
class ARLFile:
    ...
    def refresh( self, quickSearch ):
        ...
        # update numb_x_pnts & numb_y_pnts to reflect additional info in
        # the grid variable (split into two characters now xscale and yscale)
        # that indicates if one of the dimensions exceeds 999 points.
        knx = ord(file_header.gridx)
        kny = ord(file_header.gridy)
        if ((knx >= 64) or (kny >= 64)):
            index.numb_x_pnts = (knx - 64)*1000 + index.numb_x_pnts
            index.numb_y_pnts = (kny - 64)*1000 + index.numb_y_pnts
        # ===================================================================
        ...
        while True:
            ...
            if data[14:18] == "INDX":
                ...
                # the grid variable (split into two characters now xscale and yscale)
                # that indicates if one of the dimensions exceeds 999 points.
                knx = ord(header.gridx)
                kny = ord(header.gridy)
                if ((knx >= 64) or (kny >= 64)):
                    index.numb_x_pnts = (knx - 64)*1000 + index.numb_x_pnts
                    index.numb_y_pnts = (kny - 64)*1000 + index.numb_y_pnts
                # ===================================================================
                ...
                # For a quick search, hit the first two time periods to determine the interval,
    ...
    def getMergeTimePeriods( self, other, startpos=None ):
        ...
        if interval is None:
            ...
            for time in other.time:
                # If the timeperiod is exactly interval from the current position...
