...
def reproject_images(config, grid_bbox):
    ...
    images_dir = config.get('DispersionGridOutput', "OUTPUT_DIR")
    for image in os.listdir(images_dir):
        # Only translate images that include concentration data.  This should be updated to be more dynamic.
        if image.startswith('hourly_') or image.startswith('daily_maximum_') or image.startswith('daily_average_'):
            image_path = os.path.join(images_dir, image)
            tiff_path1 = os.path.join(images_dir, 'temp1.tif')
            tiff_path2 = os.path.join(images_dir, 'temp2.tif')
