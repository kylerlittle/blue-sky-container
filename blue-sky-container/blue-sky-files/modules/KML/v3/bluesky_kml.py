...
class OutputKML(Process):
    ...
    def run(self, context):
        ...
        # MakePolygons Section
        if kernel_config.get_if_exists('PolygonsKML', 'MAKE_POLYGONS_KMZ'):
            config.add_section('PolygonsKML')
            opts = [
                'MAKE_POLYGONS_KMZ', 'POLYGONS_OUTPUT_DIR',
                'MAKEPOLYGONS_BINARY', 'KMZ_FILE',
                'OVERLAY_TITLE', 'DATA_LEVELS', 'POLYGON_COLORS'
            ]
            for opt in opts:
                val = kernel_config.get_if_exists('PolygonsKML', opt)
                if val:
                    config.set('PolygonsKML', opt, val)
        ...
        config.set(sect2, 'LAYER', self.config('LAYER'))
        ...
        config.set(sect3, 'HOURLY_COLORS', self.config('HOURLY_COLORS'))
        config.set(sect3, 'THREE_HOUR_COLORS', self.config('THREE_HOUR_COLORS'))
        config.set(sect3, 'DAILY_COLORS', self.config('DAILY_COLORS'))

        # Build as many color map config sections as are specified for the
        # hourly, three hour, and daily time series
        color_map_sections = [
            config.get_if_exists("DispersionGridOutput", 'HOURLY_COLORS'),
            config.get_if_exists("DispersionGridOutput", 'THREE_HOUR_COLORS'),
            config.get_if_exists("DispersionGridOutput", 'DAILY_COLORS'),
            config.get_if_exists("PolygonsKML", 'POLYGON_COLORS')
        ]
        color_map_sections = set(functools.reduce((lambda x,y: x + y.split(',') if y else x), color_map_sections, []))
        for color_map_section in color_map_sections:
            config.add_section(color_map_section)
            if kernel_config.get_if_exists(color_map_section, 'DEFINE_RGB'):
                opts = ['DEFINE_RGB','RED','GREEN','BLUE']
                config.set(color_map_section, 'DEFINE_HEX', 'False')
            elif kernel_config.get_if_exists(color_map_section, 'DEFINE_HEX'):
                opts = ['DEFINE_HEX','HEX_COLORS']
                config.set(color_map_section, 'DEFINE_RGB', 'False')
            else:
                self.log.error('DEFINE_RGB or DEFINE_HEX must be set to true.')
            opts.append('DATA_LEVELS')

            for opt in opts:
                config.set(color_map_section, opt, kernel_config.get(color_map_section, opt))

            iof = kernel_config.get_if_exists(color_map_section, 'IMAGE_OPACITY_FACTOR')
            if iof:
                config.set(color_map_section, 'IMAGE_OPACITY_FACTOR', iof)
        ...
        config.set(sect6, 'FIRE_EVENT_CSV', self.config('FIRE_EVENT_CSV') or "")

        sect7 = 'SmokeDispersionKMLOutput'
        self._create_config_section(config, sect7, ['KMZ_FILE', 'KMZ_FIRE_FILE'])
        config.set(sect7, 'FIRE_LOCATION_CSV', os.path.join(self.config('OUTPUT_DIR'), 'data'))
        ...
        kmz_files = [
            ...
            self.config("KMZ_FIRE_FILE")
        ]
        if config.has_section('PolygonsKML') and config.getboolean('PolygonsKML', 'MAKE_POLYGONS_KMZ'):
            kmz_files.append(config.get('PolygonsKML', 'KMZ_FILE'))
        for kmz_file in kmz_files:
            kmz_final_file = os.path.join(self.config("OUTPUT_DIR"), kmz_file)
            kmz_temp_file = context.full_path(kmz_file)
